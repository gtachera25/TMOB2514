...
with market_definition as (
    select
        ...
        mkt.census_tract_code || ' || ' || fpt.state || ' || ' || fpt.provider || ' || ' || fpt.phase as market,
        fpt.pct_in_expansion_phase as pct_in_footprint, -- Switch to 1 for National Level
        mkt.county_name as county,
        mkt.cbsa_name as metro_name,
        dm.dma_name as dma_name
        -- Comment out for higher level rollups
        -- fpt.designation,
        -- fpt.name,
        -- mkt.census_county_subdivision_name as csd_name,
        -- mkt.census_county_subdivision_code as csd_code,
        -- mkt.census_county_code as county_code,
        -- mkt.county_name as county,
        -- mkt.state,
        -- mkt.cbg_morphology,
        -- mkt.ilec
    from {{ ref('report_cbs_in_expansion_phase_mapped') }} fpt
...
market_summary as (
    ...
    from base_cbs
    group by 1,2,3,4 --,2,3,4,5,6,7,8,9
...
bdc_market_provider as (
    ...
    where resi_tech_code in ('40', '50')
        and resi_down_speed >= 900
            ...
            and hocofinal not in ('Greenlight Networks, LLC', 'Future Fiber Parent, LP', 'Loop Internet')
    group by 1,2,3
),
...
select
    ...
    base.market, -- Keep original concatenated market as DISTKEY
    SPLIT_PART(base.market, ' || ', 3) as provider,
    SPLIT_PART(base.market, ' || ', 1) as census_tract_code,
    SPLIT_PART(base.market, ' || ', 4) as phase,
    base.county,
    base.metro_name,
    base.dma_name,
    SPLIT_PART(base.market, ' || ', 2) as state_name,
    -- Comment out for state rollup
    -- base.designation,
    -- base.name,
    -- base.pct_in_footprint, -- Comment out
    -- base.csd_name,
    -- base.county,
    -- base.state,
    -- base.cbg_morphology,
