...
with market_definition as (
    ...
    from {{ source('project_tmob2514','report_cbs_in_intrepid_polygon_footprint') }} fpt
...
precisely_predicted_pen as (
    select
        ...
        sum(pen.predicted_penetration_12m * pct_in_footprint * total_passings) / nullif(sum(pct_in_footprint * total_passings), 0) as predicted_penetration_12m,
        sum(pen.predicted_penetration_36m * pct_in_footprint * total_passings) / nullif(sum(pct_in_footprint * total_passings), 0) as predicted_penetration_36m,
        sum(pen.predicted_penetration_60m * pct_in_footprint * total_passings) / nullif(sum(pct_in_footprint * total_passings), 0) as predicted_penetration_60m
    from base_cbs cb
...
precisely_predicted_pen_uplift as (
    select
        cb.market,
        -- sum(unique_pbkeys) as precisely_passings,
        sum(pen_2.predicted_penetration_12m_uplift * pct_in_footprint * total_passings) / nullif(sum(pct_in_footprint * total_passings), 0) as predicted_penetration_12m_w_uplift,
        sum(pen_2.predicted_penetration_36m_uplift * pct_in_footprint * total_passings) / nullif(sum(pct_in_footprint * total_passings), 0) as predicted_penetration_36m_w_uplift,
        sum(pen_2.predicted_penetration_60m_uplift * pct_in_footprint * total_passings) / nullif(sum(pct_in_footprint * total_passings), 0) as predicted_penetration_60m_w_uplift
    from base_cbs cb
...
-- Demos
left join {{source('project_tmob2514','report_cbs_in_intrepid_polygon_footprint')}} fpt on base.market = fpt.census_block_code_2020
